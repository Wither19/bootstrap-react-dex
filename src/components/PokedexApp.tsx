...
function PokedexApp() {
  ...
  useEffect(() => {
    ...
    axios.get(url).then((response) => {
      setPokedex(_.orderBy(response.data.pokemon_entries, [pokedexSort], sortOrder));
    });
  }, []);
  ...
  const isInRegion = (id) => {
    var retValue = false;
    const currentRegion: any = regions?.find((r) => r.name.toLowerCase() == regionDropdown);
        ...
        <div style={{ flexBasis: "40%" }}>
          <div className="m-1">
            <div className="input-group">
              ...
              >
                <i
                  className={`bi bi-sort-${sortType == "dex" ? "numeric" : "alpha"}-${
                    sortOrder ? "down" : "up"
                  }`}
                ></i>
              </button>
